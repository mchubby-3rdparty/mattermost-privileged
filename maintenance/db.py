...
import config
print("Mattermost DB cleanup script. Tested on 5.33\n")
...
##################################
# enforce enhanced system policy #
##################################
print("Enforcing enhanced system policy ...")
...
cur.close()
if not hasattr(config, "enforce_enhanced_policy") or hasattr(config, "enforce_enhanced_policy") and not config.enforce_enhanced_policy:
    dbconn.rollback()
    print("\n*** rollback - not enforcing enhanced system policy ***")
elif not hasattr(config, "enforce_policy") or hasattr(config, "enforce_policy") and not config.enforce_policy:
    dbconn.rollback()
...
# system spam
cur.execute("""DELETE FROM posts WHERE type IN ('system_join_team','system_leave_team','system_add_to_team','system_remove_from_team','system_join_channel','system_leave_channel','system_purpose_change','system_header_change','system_guest_join_channel')
            OR (type IN ('system_add_to_channel','system_add_guest_to_chan') AND props::jsonb @> '{"userId":"5ugpycz1mfrj3ff4k6hbg6g37o"}'::jsonb)
            RETURNING *""")
print("* ["+("%07.6g"%round(time.time() - TS_START, 5))+"] "+str(cur.rowcount)+" deleted all system spam message(s).")

# delete autodelete-messages by message content
cur.execute("""DELETE FROM posts WHERE ispinned = false AND createat < extract(epoch from (NOW() - INTERVAL '2 day'))*1000 AND message LIKE '``BOT-AUTODELETE-FAST``%' RETURNING *""")
print("* ["+("%07.6g"%round(time.time() - TS_START, 5))+"] "+str(cur.rowcount)+" deleted old BOT-AUTODELETE-FAST message(s).")
cur.execute("""DELETE FROM posts WHERE ispinned = false AND createat < extract(epoch from (NOW() - INTERVAL '7 day'))*1000 AND message LIKE '``BOT-AUTODELETE-SLOW``%' RETURNING *""")
print("* ["+("%07.6g"%round(time.time() - TS_START, 5))+"] "+str(cur.rowcount)+" deleted old BOT-AUTODELETE-SLOW message(s).")

# delete autodelete-messages by property. We need an index to make this **really** fast...
# Dont!
...
cur.execute("""DELETE FROM posts WHERE ispinned = false AND props LIKE '%cleaner_auto_delete%' AND createat < extract(epoch from (NOW() - INTERVAL '1 day'))*1000 AND props::jsonb->>'cleaner_auto_delete' = 'day' RETURNING *""")
print("* ["+("%07.6g"%round(time.time() - TS_START, 5))+"] "+str(cur.rowcount)+" deleted AUTODELETE-PROP-DAY message(s).")
cur.execute("""DELETE FROM posts WHERE ispinned = false AND props LIKE '%cleaner_auto_delete%' AND createat < extract(epoch from (NOW() - INTERVAL '1 week'))*1000 AND props::jsonb->>'cleaner_auto_delete' = 'week' RETURNING *""")
print("* ["+("%07.6g"%round(time.time() - TS_START, 5))+"] "+str(cur.rowcount)+" deleted AUTODELETE-PROP-WEEK message(s).")


cur.close()
if not hasattr(config, "enforce_policy") or hasattr(config, "enforce_policy") and not config.enforce_policy:
    dbconn.rollback()
    print("\n*** rollback - not enforcing system policy ***")
print()
...
# posts orphaned by parent post (fixing my own shit; only happens because of AUTODELETE)
cur.execute("""DELETE FROM posts WHERE parentid IS DISTINCT FROM '' AND parentid NOT IN (SELECT id FROM posts) RETURNING *""")
